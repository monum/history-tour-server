...
def handler(event, context):
    ...
    if (
        ...
        and event["queryStringParameters"]["latitude"]
        and event["queryStringParameters"]["longitude"]
    ):
        ...
        for route in range(0, len(routes)):
            ...
            for tour_stop in range(0, len(route_deserialized["tourStops"])):
                stop_lat = float(
                    route_deserialized["tourStops"][tour_stop]["gpsCoordinates"][
                        "latitude"
                    ]
                )
                stop_lon = float(
        ...
        for k, v in top_three_routes:
            db_response = dynamodb.get_item(
                TableName=table_name, Key={"routeName": {"S": k}}
            )
            db_response_d = {
                k: deserializer.deserialize(v) for k, v in db_response["Item"].items()
            }
